...
## 2026-05-06 - [React 19 Form Actions & useActionState]
Learning: React 19 brings native support for form actions and the `useActionState` hook, enabling components to handle form submissions without manual `e.preventDefault()`, tracking loading states, or managing intermediate error states with standard `useState`.
Action: Refactored `LoginPage.tsx` to replace `useState` boilerplate for username/password tracking, error states, and `isLoading` flags with a single `useActionState` implementation. This pattern provides built-in `isPending` states and direct `FormData` access, drastically simplifying component logic and making forms more declarative.
