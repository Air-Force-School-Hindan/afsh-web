...
const LoginPage: React.FC<LoginPageProps> = ({ onLogin, onBack }) => {
    const loginAction = async (prevState: string, formData: FormData): Promise<string> => {
        ...
        return new Promise((resolve) => {
            setTimeout(() => {
                onLogin(username);
                resolve(''); // clear error if successful (though component will likely unmount)
            }, 1500);
        });
    };

    // 🛰️ Spark: React 19's useActionState simplifies form handling by natively tracking
    // the pending state and the action's return value (error message), eliminating
    // the need for manual useState hooks and controlled inputs.
            ...
            <div className="relative z-10 w-full max-w-md px-6">
                ...
                >
                    ...
                    <form action={formAction} className="space-y-6">
                        {/* Hide previous error while a new submission is pending */}
                        {error && !isPending && (
                            <motion.div
